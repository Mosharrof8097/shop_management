...
export default function CustomerLedger() {
  ...
  const [selMonth,   setSelMonth]   = useState(4);       // 0-indexed
  const [selYear,    setSelYear]    = useState('2026');
  const [exporting,  setExporting]  = useState(false);

  const allTx = mockTransactions.filter(t => t.customer_id === Number(id));
  ...
  const summary = useMemo(() => ({
    totalSale:    filtered.filter(t => t.type === 'SALE').reduce((s, t) => s + t.debit, 0),
    totalPayment: filtered.filter(t => t.type === 'PAYMENT').reduce((s, t) => s + t.credit, 0),
    openBalance:  filtered.length ? filtered[0].balance - filtered[0].debit + filtered[0].credit : 0,
    closeBalance: filtered.length ? filtered[filtered.length - 1].balance : 0,
  }), [filtered]);

  const handlePDF = async () => {
    setExporting(true);
    await new Promise(r => setTimeout(r, 120));
    const label = filterType === 'monthly'
      ? `${MONTHS[selMonth]}-${selYear}`
      : filterType === 'yearly' ? selYear : 'সম্পূর্ণ';
    await exportElementAsPDF(
      'customer-statement-content',
      `লেজার-${customer?.name}-${label}.pdf`
    );
    setExporting(false);
  };
    ...
    <div className="page">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          { label: 'পূর্ববর্তী বাকি', value: formatCurrency(summary.openBalance),  icon: <Wallet size={20}/>,        iconBg: 'bg-amber-100',   iconColor: 'text-amber-600'   },
                ...
                {filtered.map(tx => (
                  <tr key={tx.id} className={tx.type === 'PAYMENT' ? 'bg-primary-50/20' : ''}>
                    ...
                    <td className="hidden sm:table-cell">
                      {tx.invoice
                        ? <span className="font-mono text-[0.72rem] text-blue-600 font-semibold">{tx.invoice}</span>
                        : <span className="text-gray-300">—</span>}
                    </td>
                    <td className="text-right">
                      {tx.debit > 0
                        ? <span className="font-bold text-blue-700">{formatCurrency(tx.debit)}</span>
                        : <span className="text-gray-300">—</span>}
                    </td>
                    <td className="text-right">
                      {tx.credit > 0
                        ? <span className="font-bold text-primary-700">{formatCurrency(tx.credit)}</span>
                        : <span className="text-gray-300">—</span>}
                    </td>
                    <td className="text-right">
                      <span className={`font-extrabold text-[0.83rem] ${tx.balance > 0 ? 'text-red-600' : 'text-primary-600'}`}>
